...
#                                lambda a: a.any == with_parent_ix, labelmini)
...
#                            r = np.int32(r1[0] - 1 + north1[k])                      
#                            c = np.int32(r1[1] - 1 + W[k])
                                
#                            w = np.ravel_multi_index((r, c), 
#                                dims=(final_label_matrix_image2.shape), order='C')
#                            w = np.ravel_multi_index(final_label_matrix_image2,(r,c))
                            
#                            final_label_matrix_image2[r,c] = k
        ...
        if final_label_matrix_image.shape[1] > 0:
            ...
            if has_objects == 1:
                if num_objects >= 1:
#                    if any(patch_for_primary_object):
#                        ids_of_objects_to_patch = filter(
#                                lambda a: a != 0, patch_for_primary_object)
